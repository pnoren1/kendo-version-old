$(document).ready(function () {
    ...
    if (isInternetExplorer() || isEdge()) {
        ...
        MojFind("input[type=checkbox]").parent().addClass("ie-checkbox-container");
    }
    //Moj.responsive.handleTables();
    $('[data-toggle=offcanvas], .sidenav-overlay').click(function () {
        $('.sidebar-offcanvas, .sidenav-overlay, .navbar-toggle[data-toggle=offcanvas]').toggleClass('active');
        $("main").toggleClass("nav-is-visible");
        $(".main-nav-target").removeClass("open");
        $(".navbar .dropdown, .navbar-moj ul.navbar-nav").removeClass("active");
        $(".main-nav a")[0].click();
    });
    $(".navbar-moj ul.navbar-nav > li > a").click(function () {
        if ($(this).parent().attr("data-sub-nav") != 0) {
            $(".navbar-moj ul.navbar-nav").addClass('active');
            $(".navbar .dropdown").removeClass("active");
            $(this).parent().addClass("active");
        }
    });
    $(".navbar-moj .dropdown-menu > li > a").click(function () {
        if ($(this).parent().is(".see-all")) {
            $(this).closest("li.dropdown.active").removeClass('active');
            $(".navbar-moj ul.navbar-nav").removeClass('active');
        }
    });
    $(".main-nav a").click(function () {
        var e = $(this);
        if (e.attr("data-target") == "#main-nav-guidlines") {
            $("#main-nav-guidlines").empty();
            $(".guidelines").clone().appendTo("#main-nav-guidlines");
            //$("#main-nav-guidlines").append($(".guidelines"));
        }
        $(".main-nav a").removeClass("active");
        e.addClass("active");
        $(".main-nav-target").removeClass("open");
        $(e.attr("data-target")).addClass("open");
    });
    $(".td-first").live("click", function (e) {
        if (window.innerWidth <= 767) {
            var str = "", title = Resources.Messages.RowDetails;
            var grid = $(e.currentTarget).closest(".k-grid");
            var tr = $(e.currentTarget).closest("tr");
            tr.find("td").each(function (i, td) {
                if (!$(td).is(".td-action") && !$(td).is(".k-hierarchy-cell") && $(td).has("center:has(input)").length == 0 && (!$(td).attr("style") || ($(td).attr("style").indexOf("display:none") < 0 && $(td).attr("style").indexOf("display: none") < 0))) {
                    var tdTitle = $(grid.find("thead th:eq(" + i + ")")[0]).attr("data-title");
                    if (tdTitle != undefined && tdTitle.trim() != "")
                        tdTitle = tdTitle + ":";
                    var html = $(td).html();
                    //if (title == "" && html.indexOf("type=\"checkbox\"") == -1 && html.indexOf("type=\"radio\"") == -1) title = "פרטי " + html;
                    str += "<div class='col-sm-1 row-data-title'>" + tdTitle + "</div><div class='col-sm-1 row-data-txt'>" + $(td).html() + "</div>";
                }
            });
            Moj.showMessage(str, null, title, MessageType.Attention, true, null, false);
        }
    });

    $('tbody:has(.k-master-row)').live('DOMSubtreeModified', function () {
        if (window.innerWidth <= 767) {
            $(".k-detail-cell", this).attr("colspan", "2");
        }
    })
});
...
$(document).ajaxStop(function () {
    ...
    $("table .k-i-collapse,table .k-i-expand").attr("href", "javascript:void(0)");
        ...
        function parseUnobtrusive(elementId) {
            if (elementId) {
                ...
                $('input,textarea').live('change', function () {
                    ...
                    if ($(this).closest("form").length > 0 && $(this).closest("form").find('#ObjectState').length > 0)
                        $(this).closest("form").find('#ObjectState').val("true");
...
$(function () {
    ...
    $(document).delegate('.plus', 'click', function (e) {
        if ($(this).find('a').attr('disabled') != 'disabled') {
            $(e.currentTarget).closest(".moj-panel").find("#" + $(this).attr("data-id")).removeClass("hide").slideDown();
    ...
    $(document).delegate('.moj-multiselect-panel > .moj-multiselect-panel-title .txt', 'click', function (e) {
        e.stopImmediatePropagation();
        $(e.currentTarget).closest(".moj-multiselect-panel").find(".button-open-close div").trigger("click");
    });

    $(document).delegate('.plus-icon', 'click', function (e) {
        if ($(this).find('a').attr('disabled') != 'disabled') {
            $(e.currentTarget).closest(".moj-multiselect-panel").find("#" + $(this).attr("data-id")).removeClass("hide").slideDown();
            $(this).removeClass("plus-icon").addClass("minus-icon");
            $(this).closest(".moj-multiselect-panel-title").removeClass("no-border-bottom");
            $(this).find("a").attr("aria-label", $(this).find("a").attr("close-text"));
        }
    });

    $(document).delegate('.minus-icon', 'click', function (e) {
        var title = $(this).closest(".moj-multiselect-panel-title");
        $(e.currentTarget).closest(".moj-multiselect-panel").find("#" + $(this).attr("data-id")).slideUp(function () {
            title.addClass("no-border-bottom");
        });
    ...
    $('input,textarea').live('change', function () {
        ...
        if ($(this).attr('type') == "text")
            ...
            MojFind("[name='" + name + "']:checked").closest('.col, [class*="col-"]').next().addClass("bold");
            MojFind("[name='" + name + "']:not(:checked)").closest('.col, [class*="col-"]').next().removeClass("bold");
        }
    ...
    $('input[readonly], textarea[readonly]').live('focus', function () {
        if (isInternetExplorer()) // in chrome we want to copy the content of readonly fields. in IE there is a bug in this case
            $(this).trigger('blur');
    });
    ...
    return this.map(function () {
        ...
        .map(function (i, elem) {
            ...
            if (this.type == "textarea") {
                if (typeof (CKEDITOR) != "undefined" && CKEDITOR.instances[$(this).attr('id')] != undefined)
...
$.fn.serializeObjectForGrid = function () {
    ...
    for (var i = 0; i < selectElements.length; i++) {
        ...
        selectElementsArray.push(selectElements[i].name);
        selectItemsArray.push(selectedOptions);
    }

    $.each(array, function () {
        if (selectElementsArray.indexOf(this.name) != -1)
            this.value = selectItemsArray[selectElementsArray.indexOf(this.name)];
...
$.fn.visible = function (isVisible) {
    if (this.length > 0) {
        ...
        if (wrapDiv.length > 0) {
            ...
            wrapDiv = $(this[0]).closest("div[class*='col-sm']");
            if (wrapDiv.length > 0) {
                mainDiv = wrapDiv;
                className = "hide";
            }
...
$.fn.hasScrollBar = function () {
    var parentGrid = $(this).parents(".moj-extended-grid");
    var isParentGridHide = parentGrid.css('display') == 'none';
    if (parentGrid.length > 0 && isParentGridHide)
        parentGrid.show();
    var scrollHeight = this.get(0).scrollHeight;
    var height = this.height();
    if (isParentGridHide)
        $(this).parents(".moj-extended-grid").hide();
    ...
    if (grid != undefined) {
        ...
        if (checkAll.length > 0) {
            ...
            $(grid.dataSource.data()).each(function () {
                if (!Moj.isTrue(this[name]))
                    ifCheckAll = false;
            });
            if (ifCheckAll && grid.dataSource.data().length > 0)
...
var Moj = {
    ...
    HtmlHelpers: {
        //Buttons
        getDropDownsClienDataSource: function (cachObjectName, controlId, controlType) {
            if (dropDownsCach[cachObjectName] != undefined) {
                MojFind('#' + controlId).data(controlType).setDataSource(dropDownsCach[cachObjectName]);
            }
            else {
                Moj.safeAjaxCall('/Moj/GetDropDownData?dropDownName=' + cachObjectName, undefined, undefined, undefined, function (data) {
                    dropDownsCach[cachObjectName] = dropDownsCach[cachObjectName] || data;
                    MojFind('#' + controlId).data(controlType).setDataSource(dropDownsCach[cachObjectName]);
                })
            }
        },
        ...
        _onSubmitButtonClicked: function (e, url, onSuccess) {
            ...
            $.each(MojFind('textarea'), function () {
                if (typeof (CKEDITOR) != "undefined" && CKEDITOR.instances[$(this).attr('id')] != undefined) {
                    CKEDITOR.instances[$(this).attr('id')].updateElement();
                    //CKEDITOR.instances[$(this).attr('id')].destroy();
                    $(this).val($('<div/>').text($(this).val()).html());
                }
            });
        ...
        _onCancelButtonClicked: function (e) {
            ...
            if (myWindow.length > 0) {
                ...
                Moj.controlsPopupsClean(true, false, null);
        ...
        _cancelInGrid: function (gridName, methodNameAfterCancel) {
            Moj.controlsPopupsClean(false, true, gridName);
        ...
        _editRowInGridInline: function (e, gridName, url, dataItem, currentTr) {
            ...
            //colapse details
            MojFind("[id^='" + gridName + "']").find(".k-hierarchy-cell .k-i-collapse").not(".current-open").trigger("click");
        ...
        _showGridDetails: function (e) {
            if ($(e.currentTarget).attr("disabled") != "disabled") {
                ...
                if (MojFind('#tr_' + gridName + '_Details').length > 0) {
                    ...
                    if (1==1 || isEditableGridInlineAttr == undefined || Moj.isFalse(isEditableGridInlineAttr)) {
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    if (frm.length > 0) {
                                        var action = frm.attr("id").replace("frm", "");
                                        MojFind(MojFind("[id^='btn'][id*='" + action + "']")).parent("div").parent("div").addClass("hide");
                                        parent.postMessage({ editOpen: true }, '*'); 
                                    }
                                }
                            });
        ...
        _editRowInTemplate: function (e, gridName, editUrl, dataItem, currentTr) {
            var rowToExpand = $(e.currentTarget).closest("tbody tr").find(".k-hierarchy-cell .k-i-expand");
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.Errors != undefined) {
                            ...
                            MojFind('#div_' + gridName + '_Details').html("");
                        }
                        else {
                            MojFind('#div_' + gridName + '_Details').html(data);
        ...
        _deleteVirtualRowGrid: function (e) {
            if ($(e.currentTarget).attr("disabled") != "disabled") {
                var gridName = e.currentTarget.closest('[data-role=grid]').id;
                if (MojFind('#tr_' + gridName.split('-')[0] + '_Details').length > 0) {
                    Moj.showMessage(window.Resources.Messages.InlineNewEditSaveMessage);
                    return false;
                }
                var grid = $(e.currentTarget).closest(".k-grid").data("kendoGrid");
                ...
                if (!setAllModel) {
                    ...
                    for (var i = 0; i < o.length; i++) {
                        ...
                        if (name.indexOf(".") != -1) {
                            ...
                            var lengthName = name.split(".").length;
                            name = name.split(".")[lengthName - 1];
                        }
        ...
        _scrollableGridDataBound: function (e) {
            ...
            var maxHeight = e.element.attr("max_height") || e.element.attr("max-height");
            var maxRecordNumber = e.element.attr("max_record_number") || e.element.attr("max-record-number");
        ...
        _onGridDetailActionExpand: function (e) {
            e.masterRow.find(".k-hierarchy-cell .k-i-collapse").addClass("current-open");
            e.sender.table.find(".k-hierarchy-cell .k-i-collapse").not(".current-open").trigger("click");
            if (Moj.isTrue(e.masterRow.find(".td-action.edit-row a").attr("isDisabledRowsInEditTemplate")))
                e.sender.table.find(".k-hierarchy-cell .k-i-expand").attr("disabled", "disabled");
            e.masterRow.find(".k-hierarchy-cell .k-i-collapse").removeClass("current-open");
        },

        _onGridDetailActionCollapse: function (e) {
            e.detailRow.remove();
            if (Moj.isTrue(e.masterRow.find(".td-action.edit-row a").attr("isDisabledRowsInEditTemplate")))
                e.sender.table.find(".k-hierarchy-cell .k-i-expand").removeAttr("disabled");
        },
        ...
        _onSubGridInit: function (e) {
            ...
            var setData = function () {
                ...
                if (typeof (grid) != "undefined") {
                    var list = [];
                    if (subListName.includes("_")) {
                        var a = subListName.split("_");
                        list = e.data[a[0]][a[1]];
                    }
                    else {
                        list = e.data[subListName];
                    }
        ...
        _onMultiDropDownChanged: function (e) {
            ...
            e.sender.element.parent().attr("title", selectedTextItems.substring(0, selectedTextItems.length - 2));
            e.sender.input.attr("title", selectedTextItems.substring(0, selectedTextItems.length - 2));
        ...
        _onIncreaseQuantity: function (increaseButtonName, countField, textField) {
            var id = increaseButtonName.replace("btnIncreaseQuantity", ""); // id + "_" + index + "__" + keyField
            var txtCurrentQuantity = MojFind("#" + id);
            var prevQuantity = parseInt(txtCurrentQuantity.val())

            if (isNaN(prevQuantity))
                prevQuantity = 0;

            MojControls.TextBox.setValue(txtCurrentQuantity, prevQuantity + 1);
            Moj.HtmlHelpers._changeTitle($(MojFind("#" + increaseButtonName).parents(".moj-multiselect-panel")).attr("id"), countField, textField);

            if (prevQuantity + 1 > 0) {
                MojFind("#" + increaseButtonName.replace("btnIncreaseQuantity", "btnDecreaseQuantity")).enable(true);
            }
        },

        _onDecreaseQuantity: function (decreaseButtonName, countField, textField) {
            var id = decreaseButtonName.replace("btnDecreaseQuantity", ""); // id + "_" + index + "__" + keyField
            var txtCurrentQuantity = MojFind("#" + id);
            var prevQuantity = parseInt(txtCurrentQuantity.val())

            MojControls.TextBox.setValue(txtCurrentQuantity, prevQuantity - 1);
            Moj.HtmlHelpers._changeTitle($(MojFind("#" + decreaseButtonName).parents(".moj-multiselect-panel")).attr("id"), countField, textField);

            if (prevQuantity - 1 == 0) // replace quantity
            {
                MojFind("#" + decreaseButtonName).enable(false);
            }
        },

        _onChangePlusMinusTextBox: function (PlusMinusTextBoxName, countField, textField) {
            var currentText = parseInt(MojFind("#" + PlusMinusTextBoxName).val());

            if (isNaN(currentText) || currentText < 0) {
                MojFind("#" + PlusMinusTextBoxName).val("");
                MojFind("#btnDecreaseQuantity" + PlusMinusTextBoxName).enable(false);
            }
            else if (currentText == 0) {
                MojFind("#btnDecreaseQuantity" + PlusMinusTextBoxName).enable(false);
            }
            else if (currentText > 0) {
                MojFind("#btnDecreaseQuantity" + PlusMinusTextBoxName).enable(true);
            }
            Moj.HtmlHelpers._changeTitle($(MojFind("#" + PlusMinusTextBoxName).parents(".moj-multiselect-panel")).attr("id"), countField, textField);
        },

        _changeTitle: function (panelId, countField, textField) {

            var title = "";
            var count = 0;

            //loop over labels
            MojFind("#" + panelId).find("span[id*='__" + textField + "']").each(function (index, item) {
                var actionCount = parseInt(MojFind("#" + $(item).attr("id").replace(textField, countField)).val());
                if (MojFind("#" + panelId).data("mojPanelMultiSelect") != undefined) {
                    MojFind("#" + panelId).data("mojPanelMultiSelect").dataSource[index][countField] = actionCount;
                }

                if (actionCount > 0) {
                    title += $(this).html() + " (" + actionCount + "), ";
                    $(item).parent().addClass("moj-bold");
                    count += actionCount;
                }
                else {
                    $(item).parent().removeClass("moj-bold");

                }
            });

            if (title != "")
                title = title.substr(0, title.length - 2); // cut the last ", "

            var titleControl = MojFind("#" + panelId).find(".moj-multiselect-panel-title").find(".txt");

            titleControl.html(title);

            if (titleControl[0].scrollHeight > titleControl.innerHeight()) {

                title = "נבחרו " + count + "  פעולות";
                titleControl.html(title);
            }
            MojFind("#" + panelId).find("legend").html(title);
        },
        ...
        _showCaptchaWindowIfNeeded: function (xhr) {
            if (typeof (xhr.responseText) != "undefined" && xhr.responseText.indexOf("CaptchaScript") > -1 && $(Moj.Constants["selectedTabContent"]).attr("id") != 'frmCaptcha') {
                Moj.website.openPopupWindow('frmCaptcha', '', '', 390, 275, true, true, false, baseUrl + '/Home/Captcha', '');
            }
        },

        openWindowByPost: function (url, params, httpMethod, target, isOpenInFullWindow) {
            ...
            if (params) {
                Moj.HtmlHelpers.setHiddensToForm(tempForm, "", params);
            }

            if (isOpenInFullWindow) {
                ...
                window.open("", "Map", "status=0,title=0,height=" + height + ",width=" + width + ",scrollbars=1");
        ...
        setHiddensToForm: function (tempForm, objName, params) {
            $.each(params, function (key, value) {
                if (value) {
                    if (value.constructor == Object && Object.keys(value).length > 0) {
                        Moj.HtmlHelpers.setHiddensToForm(tempForm, key + ".", value);
                    }
                    else if (value.constructor == Array && Object.keys(value).length > 0) {
                        $.each(value, function (index, item) {
                            if (item.constructor != Object && item.constructor != Array) {
                                item = { "": item };
                                tempObjName = objName + key + "[" + index + "]";
                            }
                            else {
                                tempObjName = objName + key + "[" + index + "].";
                            }
                            Moj.HtmlHelpers.setHiddensToForm(tempForm, tempObjName, item);
                        });
                    }
        ...
        _setDirection: function (e) {
            ...
            if (e.srcElement.classList.contains("k-i-ltr")) {
                $("a:has(.k-i-ltr)").addClass("moj-disabled");
                $("a:has(.k-i-rtl)").removeClass("k-state-selected").removeClass("moj-disabled");
            }
            else {
                $("a:has(.k-i-rtl)").addClass("moj-disabled");
                $("a:has(.k-i-ltr)").removeClass("k-state-selected").removeClass("moj-disabled");
            }
        ...
        _backToListGrigForSubmit: function (gridName) {
            ...
            MojFind("[id^='" + gridName + "'] tr.hide").removeClass("hide");
            //For template grid
            MojFind("[id^='" + gridName + "'] .k-hierarchy-cell .k-i-collapse").trigger("click");
            MojFind("[id^='" + gridName + "'] .k-hierarchy-cell .k-i-expand").removeAttr("disabled");
            //if exist other div-buttons
            if (MojFind(".div-buttons").find("div[id^='div_btnSaveInGrid'],div[id^='div_btnCancelInGrid']").not("[class*=hide]").length == 0) {
                var frm = MojFind('#div_' + gridName + '_Details').closest("form");
                if (frm.length > 0) {
                    ...
                    parent.postMessage({ editOpen: false }, '*');
                }
            }
        },
        ...
        openDropDown: function () {
            ...
            if (data[0] != undefined && data[0].IsActive != undefined) {
        ...
        removeNotActive: function (e) {
            ...
            if (data.length && data[0].IsActive != undefined) {
                for (var item = data.length - 1; item >= 0; item--) {
                    if (data[item].IsActive == false) {
                        if (data[item][dropDown.options.dataValueField] == dropDown.element.attr('originalIndex')) {
        ...
        _getFilteredActiveData: function (data, originalIndex, dropDown) {
            var newData = [];
            if (data.length && data[0].IsActive != undefined) {
                for (var item = data.length - 1; item >= 0; item--) {
                    if (data[item].IsActive == false) {
                        if (data[item][dropDown.options.dataValueField] == originalIndex) {
                            newData.unshift(data[item]);
                        }
                    }
                    else {
    ...
    setWindowTitle: function (kendoWindow, title) {
        $(kendoWindow).closest('div').siblings().find('.k-window-title').html(title);
    },

    controlsPopupsClean: function (isDetails, isGridForSubmit, gridName) {

        //for grid details (not for sumbit) + cancel button in the tabs (general cancel button)
        if (Moj.isTrue(isDetails)) {

            if (MojFind("#DetailsContent").length != 0) //grid details (not for sumbit)
            {
        //        MojFind("#DetailsContent").find("input[data-role='combobox']").each(function () { if ($(this).data('kendoComboBox') != undefined) { $(this).data('kendoComboBox').destroy(); } });
        //        MojFind("#DetailsContent").find("input[data-role='dropdownlist']").each(function () { if ($(this).data('kendoDropDownList') != undefined) { $(this).data('kendoDropDownList').destroy(); } });
                  MojFind("#DetailsContent").find("input[data-role='datetimepicker']").each(function () { if ($(this).data('kendoDateTimePicker') != undefined) { $(this).data('kendoDateTimePicker').destroy(); } });
                  MojFind("#DetailsContent").find("input[data-role='datepicker']").each(function () { if ($(this).data('kendoDatePicker') != undefined) { $(this).data('kendoDatePicker').destroy(); } });
                  MojFind("#DetailsContent").find("select[data-role='multiselect']").each(function () { if ($(this).data('kendoMultiSelect') != undefined) { $(this).data('kendoMultiSelect').destroy(); } });

            }
            else //cancel button in the tabs (general cancel button)
            {
        //        MojFind("input[data-role='combobox']").each(function () { if ($(this).data('kendoComboBox') != undefined) { $(this).data('kendoComboBox').destroy(); } });
        //        MojFind("input[data-role='dropdownlist']").each(function () { if ($(this).data('kendoDropDownList') != undefined) { $(this).data('kendoDropDownList').destroy(); } });
                  MojFind("input[data-role='datetimepicker']").each(function () { if ($(this).data('kendoDateTimePicker') != undefined) { $(this).data('kendoDateTimePicker').destroy(); } });
                  MojFind("input[data-role='datepicker']").each(function () { if ($(this).data('kendoDatePicker') != undefined) { $(this).data('kendoDatePicker').destroy(); } });
                  MojFind("select[data-role='multiselect']").each(function () { if ($(this).data('kendoMultiSelect') != undefined) { $(this).data('kendoMultiSelect').destroy(); } });
            }

        }
        else if (Moj.isTrue(isGridForSubmit)) // for grid for submit - inline  + list details
        {

        //    MojFind("#div_" + gridName + "_Details").find("input[data-role='combobox']").each(function () { if ($(this).data('kendoComboBox') != undefined) { $(this).data('kendoComboBox').destroy(); } });
        //    MojFind("#div_" + gridName + "_Details").find("input[data-role='dropdownlist']").each(function () { if ($(this).data('kendoDropDownList') != undefined) { $(this).data('kendoDropDownList').destroy(); } });
              MojFind("#div_" + gridName + "_Details").find("input[data-role='datetimepicker']").each(function () { if ($(this).data('kendoDateTimePicker') != undefined) { $(this).data('kendoDateTimePicker').destroy(); } });
              MojFind("#div_" + gridName + "_Details").find("input[data-role='datepicker']").each(function () { if ($(this).data('kendoDatePicker') != undefined) { $(this).data('kendoDatePicker').destroy(); } });
              MojFind("#div_" + gridName + "_Details").find("select[data-role='multiselect']").each(function () { if ($(this).data('kendoMultiSelect') != undefined) { $(this).data('kendoMultiSelect').destroy(); } });

        //    MojFind("#tr_" + gridName + "_Details").find("input[data-role='combobox']").each(function () { if ($(this).data('kendoComboBox') != undefined) { $(this).data('kendoComboBox').destroy(); } });
        //    MojFind("#tr_" + gridName + "_Details").find("input[data-role='dropdownlist']").each(function () { if ($(this).data('kendoDropDownList') != undefined) { $(this).data('kendoDropDownList').destroy(); } });
              MojFind("#tr_" + gridName + "_Details").find("input[data-role='datetimepicker']").each(function () { if ($(this).data('kendoDateTimePicker') != undefined) { $(this).data('kendoDateTimePicker').destroy(); } });
              MojFind("#tr_" + gridName + "_Details").find("input[data-role='datepicker']").each(function () { if ($(this).data('kendoDatePicker') != undefined) { $(this).data('kendoDatePicker').destroy(); } });
              MojFind("#tr_" + gridName + "_Details").find("select[data-role='multiselect']").each(function () { if ($(this).data('kendoMultiSelect') != undefined) { $(this).data('kendoMultiSelect').destroy(); } });
        }

        else { // for loadMenuTab - tab and for PDO.reloadEntityContentTab func
        //    MojFind("input[data-role='combobox']").each(function () { if ($(this).data('kendoComboBox') != undefined) { $(this).data('kendoComboBox').destroy(); } });
        //    MojFind("input[data-role='dropdownlist']").each(function () { if ($(this).data('kendoDropDownList') != undefined) { $(this).data('kendoDropDownList').destroy(); } });
              MojFind("input[data-role='datetimepicker']").each(function () { if ($(this).data('kendoDateTimePicker') != undefined) { $(this).data('kendoDateTimePicker').destroy(); } });
              MojFind("input[data-role='datepicker']").each(function () { if ($(this).data('kendoDatePicker') != undefined) { $(this).data('kendoDatePicker').destroy(); } });
    ...
    renderRecaptcha: function (siteKey, divId, recaptchaUrl) {
        var callbackFunc = 'recaptchaCheckCallback';
        var expiredCallbackFunc = 'recaptchaExpiredCallback';
        if (typeof grecaptcha != 'undefined' && typeof grecaptcha.render == 'function') {
            grecaptcha.render(divId, { 'sitekey': siteKey, 'callback': callbackFunc, 'expired-callback': expiredCallbackFunc });
        }
        else {
            if ($('head').find('#recptchaScript').length == 0) {
                ...
                script.async = true;
                script.defer = true;
                script.id = 'recptchaScript';
                script.addEventListener('load', function () {
                    Moj.renderRecaptcha(siteKey, divId, recaptchaUrl, callbackFunc, expiredCallbackFunc);
                });
                document.getElementsByTagName("head")[0].appendChild(script);
            }
            else {
                //wait until the grecaptcha to be available
                var handlerNumber = setTimeout(function () {
                    Moj.renderRecaptcha(siteKey, divId, recaptchaUrl, callbackFunc, expiredCallbackFunc);
                    clearTimeout(handlerNumber);
                }, 10);
            }
        }
    },
    ...
    changeObjectsState: function (control) {
        if (control.attr('ObjectStateEntity') != undefined && control.closest("form").length > 0) {
            control.closest("form").find('#ObjectState' + control.attr('ObjectStateEntity')).val("true");
        }
        if (control.closest("form").length > 0 && control.closest("form").find('#ObjectState').length > 0) {
            control.closest("form").find('#ObjectState').val("true");
        }
    },
        ...
        for (var i in object) {
            if (typeof (object[i]) == "function") {
                ...
                if (typeof objectValue == "string" && objectValue)
                    objectValue = Moj.HtmlHelpers._htmlEscape(objectValue);
                selector.append("<input type='hidden' name='" + name + Moj.getNameObjectForSubmit(i, name) + "' value='" + objectValue + "'/>");
            }
        }
    },
    ...
    showMessage: function (data, okFunction, title, type, isCloseWindowOnEnter, width, withIcon) {
        ...
        data = "<div class='moj-window-message-data' id='msgDescription'>" + data + "</div>";
        ...
        if (withIcon != undefined && withIcon != null && Moj.isFalse(withIcon))
            iconAlert = "";
        ...
        kendoWindow.removeAttr("tabindex");
    ...
    confirm: function (message, okFunction, parameter, cancelFunction, title, isCloseWindowOnEnter, okText, cancelText, messageType, width) {
        ...
        message = "<div class='moj-window-message-data' id='msgDescription'>" + message + "</div>";
        var designLine = '<div class="clear-both"></div><div class="moj-window-line"></div>';
    ...
    clearFields: function (e, selector, filter) {
        ...
        MojFind(selector + " span.k-combobox input[data-role='combobox']" + filter).each(function () {
            $(this).data("kendoComboBox").dataSource.filter([]);
    ...
    checkChanges: function (isCanOpenMethodName, id, menuId) {
        ...
        if (MojFind("[id^='content']").children.length > 1) {
            ...
            $(forms).each(function () {
                var formId = this.id;
                if (MojFind('#' + formId).find("[id^='ObjectState']:input[value='true']").length > 0 && formId.toLowerCase().indexOf("search") == -1 && formId.toLowerCase().indexOf("list") == -1) {
                    isShowConfirm = true;
                }
            });
    ...
    addTab: function (selectorTabStrip, text, id, data, isCenterContent, callBack, reload, cssClass, isSubItem, closeTabCallback) {
        ...
        if (length < 6 && !existTab) {
            ...
            tabStrip.insertAfter(
                {
                    text: text,
                    content: ("<div  " + centerClass + " id='divContent_" + id + "'><center><img id='loading' class='margin-top30' alt='' src='" + baseUrl + "/Content/kendo/Default/loading-image.gif' /></center></div>")
                },
            ...
            callBack();
            $(selectorTabStrip).find('li').eq(tabIndex).on("tabClosed", closeTabCallback);
    ...
    closeTab: function (selectorTabStrip, closeTabId) {
        ...
        if (result == true) {
            ...
            $("#" + closeTabId).closest("li").trigger("tabClosed");
    ...
    website: {
        ...
        setScrollMsg: function () {
            MojFind(".ul-scroll li").each(function () {
                MojFind(".ul-paging-message").append("<a href='javascript:void(0)'><li></li></a>");
            });
            ...
            scrollMessage(MojFind(".ul-paging-message a:first-child"));
            ...
            MojFind(".ul-paging-message a").click(function () {
                scrollMessage($(this));
            });

            MojFind(".ul-paging-message li").click(function () {
                scrollMessage($(this).parent());
            });
    ...
    wizard: {
        ...
        activateMainMenuTab: function (selector) {
            ...
            MojFind(".wizrad-stage").html($("#" + selector + ".main-menu-item .main-menu-title span").html() + "<span>" + ($("#" + selector + ".main-menu-item").index() + 1) + " מתוך " + $(".main-menu-item").length + "</span>");
        },
        showSubMenu: function (selector, e, isMainMenuCalled) {
            ...
            if (MojFind("#" + selector).has(".ul-sub-menu").length > 0) {
                if (isMainMenuCalled) {
                    ...
                    MojFind(".sub-menu-container").removeClass("hide").show();
                    MojFind("#content").empty();
                    //check if from prev or next
                    if (Moj.wizard.fromPrevClick)
                        MojFind(".sub-menu-back ul li:not(.hide):last div").click();
                    else
                        MojFind(".sub-menu-back ul li:not(.hide):first div").click();
            ...
            Moj.wizard.fromPrevClick = false;
            return;
        },
        ...
        loadActionTab: function (e, isVisibleContentTab, isDataFromParams, callBack) {
            ...
            if (loadContent && !isSubMenu) {
                ...
                if (Moj.isTrue($(e).attr("data-submit-behavior")) && !Moj.isTrue(isDataFromParams))
                    data = JSON.stringify($($(e).closest("form")).serializeObject());
                $.ajax({
                    type: "POST",
        ...
        _setContentTab: function (contentTabId, e) {
            ...
            MojFind("#" + contentTabId).removeClass("hide").show().addClass("active").attr("aria-hidden", "false");
                ...
                if (Moj.isTrue(hasSubMenu)) {
                    ...
                    if (typeof (prevActionLink) == "undefined" || prevActionLink.length == 0) {
                        ...
                        $(MojFind(".ul-main-menu > li:not([disabled])").get().reverse()).each(function (i, e) {
                            if ($(e).index() < $("li.main-menu-item.active", currentUl).index() && $(e).attr("disabled") != "disabled") {
                    ...
                    currentLi = Moj.isTrue(fromFocused) ? $("li.main-menu-item" + activeSelector, currentUl).closest("li") : $("li.main-menu-item.active", currentUl);
                    $(MojFind(".ul-main-menu > li:not([disabled])").get().reverse()).each(function (i, e) {
                        if ($(e).index() < $(currentLi).index() && $(e).attr("disabled") != "disabled") {
                            prevActionLink = $("div.main-menu-title", e);
                            return false;
        ...
        _disabledNextButton: function (isHide) {
            ...
            if (isHide) {
                ...
                if (MojFind(".main-menu").data("mojWizard") != undefined && Moj.isTrue(MojFind(".main-menu").data("mojWizard").IsSubmitEndProcsess)) {
                    MojFind("#div_btnWizardAction").removeClass("hide").show();
                }
            }
        },
        _enableNextButton: function () {
            ...
            MojFind("#div_btnGoToNextView").removeClass("hide").show();
        ...
        _enablePrevButton: function () {
            ...
            MojFind("#div_btnGoToPrevView").removeClass("hide").show();
        },
            ...
            if (Moj.isTrue(isEnable) || typeof (isEnable) == "undefined") {
                ...
                if (MojFind(".ul-metro li").size() > 0) {
                    ...
                    if (hasSubMenu) {
                        ...
                        currentTab = MojFind(".ul-main-menu[wizard='True'] li.main-menu-item.active");
                        tabsCount = MojFind(".ul-main-menu[wizard='True'] li.main-menu-item").length;
                        activeIndex = currentTab.index();
        ...
        initWizard: function () {
            var menuItemsLength = MojFind(".ul-main-menu > li").length;
            if (menuItemsLength > 5) {
                MojFind(".ul-main-menu").addClass("multi-steps");
            }
            else if (menuItemsLength == 1) {
                MojFind(".ul-main-menu").addClass("one-step");
            }
        ...
        hideSubMenuItem : function (id) {
            $('[id=submenu_' + id + '_div]').attr('disabled', 'disabled').addClass('hide');
        },
        showSubMenuItem: function (id) {
            $('[id=submenu_' + id + '_div]').removeAttr('disabled').removeClass('hide');
        }
    },
    ...
    openFirstGridDetailAction: function (gridName) {
        var grid = MojFind("[id^='" + gridName + "']");
        var firstRowToExpand = grid.find("tbody tr").eq(0).find(".k-hierarchy-cell .k-i-expand");
    ...
    openNextGridDetailAction: function (gridName) {
        ...
        var nextRowToExpand = grid.find(".k-hierarchy-cell .k-i-collapse").closest("tr").next().next().find(".k-hierarchy-cell .k-i-expand");
        if (nextRowToExpand.length > 0) {
            kgrid.select(grid.find(".k-hierarchy-cell .k-i-collapse").closest("tr").next().next());
            ...
            if (nextPager.length > 0) {
                ...
                grid.find(".k-master-row").eq(0).find(".k-hierarchy-cell .k-i-expand").trigger("click");
            } else {
                kgrid.select(grid.find(".k-master-row"));
                grid.find(".k-master-row").find(".k-hierarchy-cell .k-i-collapse").trigger("click");
            }
        }
    },
    ...
    collapseGridDetailAction: function (gridName) {
        var grid = MojFind("[id^='" + gridName + "']");
        var rowToExpand = grid.find(".k-hierarchy-cell .k-i-collapse");
        if (rowToExpand.length > 0) {
            rowToExpand.trigger("click");
        }
    },
    ...
    initBreadCrumbs: function () {
        if ($(".container.bread-crumbs").length > 0) {
            var currentAction = $(".container.bread-crumbs").attr("data-action");
            if ($("a[data-href$='" + currentAction + "'],a[href$='" + currentAction + "']").closest("ul").closest("li").find("a").length > 0)
                $(".container.bread-crumbs").append("<li>" + $("a[data-href$='" + currentAction + "'],a[href$='" + currentAction + "']").closest("ul").closest("li").find("a").first().clone().html() + "</li>");
            if ($("a[data-href$='" + currentAction + "'],a[href$='" + currentAction + "']").closest("li").find("a").length > 0) {
                $(".container.bread-crumbs").append("<li>" + $("a[data-href$='" + currentAction + "'],a[href$='" + currentAction + "']").closest("li").find("a").parent().html() + "</li>");
            }
        }
    },

    initFooterLinks: function () {
        $.getJSON(baseUrl + "/Scripts/Common/moj-sites.json", function (data) {
            $(data["moj-sites"]).each(function (i, e) {
                $(".ftrLnks ul").append("<li class='offscreen'>|</li><li class='offscreen'><a target='_blank' title='" + e.name + "' href='" + e.url + "'>" + e.name + "</a></li>");
            });
        });
    },
    ...
    responsive: {
        handleLtrElements: function () {
            $(".ltr-elements:not(.modified)").each(function () {
                $(this).addClass("modified");
                //var ratio = 12 / +this.className.substr(this.className.indexOf("col-sm-"), 9).split("col-sm-")[1];
                //if ($.isNumeric(ratio)) {
                $("[class*='col-sm-']", this).each(function (i, e) {
                    var cls = e.className.substr(e.className.indexOf("col-sm-"), 9);
                    //var innerRatio = +cls.split("col-sm-")[1] * ratio;
                    //$(e).removeClass(cls).addClass("col-xs-" + innerRatio);
                    $(e).removeClass(cls).addClass(cls.replace("col-sm", "col-xs"));
                });
                //}
            });
        },
        handleTables: function () {
            $("thead tr").each(function () {
                $("th:not([style*='display:none']):not([style*='display: none']):not(.k-hierarchy-cell)", this).each(function (i, e) {
                    //if (i == 0)
                    //    $(this).addClass("td-first");
                    if (i > 1 && $(this).attr("data-field") != undefined) {
                        if (!$($(this).closest("table").find("tbody tr td:not(.k-hierarchy-cell):eq(" + $(this).attr("data-index") + ")")[0]).is(".td-action")) {
                            $(this).addClass("td-hide");
                            $(this).closest("table").find("col:eq(" + i + ")").addClass("td-hide");
                        }
                        else {
                            $(this).removeClass("td-hide");
                            $(this).closest("table").find("col:eq(" + i + ")").removeClass("td-hide");
                        }
                    }
                });
            });
            ...
            $("tbody tr").each(function () {
                if ($(this).is(".tr-mode-inline-grid") && window.innerWidth <= 767) {
                    var grid = $(this).closest(".k-grid");
                    $("td:not(.td-action)", this).wrapAll("<td class='td-for-inline' colspan=\"2\"><div class='td-for-inline-details'></div></td>");
                    $(".td-for-inline-details td").each(function (i, td) {
                        if ((!$(td).attr("style") || ($(td).attr("style").indexOf("display:none") < 0 && $(td).attr("style").indexOf("display: none") < 0))) {
                            var tdTitle = $(grid.find("thead th:not([style*='display:none']):not([style*='display: none']):eq(" + i + ")")[0]).attr("data-title");
                            $(this).replaceWith("<div class='col-sm-1'>" + tdTitle + ":</div><div class='col-sm-1'>" + $(this).html() + "</div>");
                        }
                    });
                }
                else if (!$(this).is(".k-detail-row")) {
                    $("td:not([style*='display:none']):not([style*='display: none']):not(.k-hierarchy-cell)", this).each(function (i, e) {
                        if (i == 0)
                            $(this).addClass("td-first");
                        if (i > 1 && !$(this).is(".td-action")) {
                            $(this).addClass("td-hide");
                            $(this).closest("table").find("col:eq(" + i + ")").addClass("td-hide");
                        }
                        else {
            ...
            $("tfoot tr").each(function () {
                $("td", this).each(function (i, e) {
                    if ($($(this).closest("table").find("thead th:eq(" + i + ")")[0]).is(".td-hide")) {
                        $(this).addClass("td-hide");
                    }
                    else
                        $(this).removeClass("td-hide");
            ...
            $("td:not(.td-for-inline):not(.k-detail-cell) [class*='col-sm-']").removeClass("col-sm-1 col-sm-2 col-sm-3 col-sm-4");
        }
    }
};
...
Moj.addConstant("selectedTabContent", document);
Moj.addConstant("IsInAction", false);
Moj.addConstant("DocumetFormats", "png,pdf,doc,docx,txt,gif,jpeg,jpg,bmp,ppt,pptx,tif,tiff,xls,xlsx,csv,xml,zip,msg,html,htm,mp3,mp4");
